...
def get_atom_valence(atom):
    ...
    for bond in atom.GetBonds():
        ...
        if bond_type == Chem.rdchem.BondType.SINGLE:
            ...
            explicit_valence += 1.5  # 芳香键近似
    
    # 根据元素类型判断饱和状态
    if element == 'C':
        expected_valence = 4
        is_saturated = (explicit_valence == expected_valence)
    elif element == 'N':
        expected_valence = 3 if formal_charge == 0 else 5
        is_saturated = (explicit_valence == expected_valence)
    elif element == 'O':
        expected_valence = 2
        is_saturated = (explicit_valence == expected_valence)
    else:
        is_saturated = True  # 未知元素
...
def check_atom_saturation(mol):
    """
    完全使用替代方法的饱和状态检查
    """
    saturation_info = []
    for atom in mol.GetAtoms():
        saturation_info.append(get_atom_valence(atom))
    allv = True
    for dictv in saturation_info:
        if dictv['is_saturated']==False:
            allv = False 
            break
    return allv#saturation_info
