...
def sigmaCopt(delta_dIS,delta_dFS,output = None):
    ...
    if output == None:
        ...
        if delta_dIS < 0 and delta_dFS > 0:
            ...
            ValueError("Error in sigmaCopt calculation!")
        return out
def directionality_sigma(ris,rfs,rcopt,r1,bondids,outshow=None):
    """
    sigma_Ri,diff_IS_Ri,diff_FS_Ri = directionality_sigma(R1Copt,Rref,RiCopt,Ri)
    计算delta_dIS和delta_dFS
    计算方向性
    """
    diff_is = Euclidean_distance(rcopt,ris,bondids) - Euclidean_distance(r1,ris,bondids)
    diff_fs = Euclidean_distance(rcopt,rfs,bondids) - Euclidean_distance(r1,rfs,bondids)
...
class RDA_S():
    def __init__(self,ISfile,FSfile,path,ids):
        self.IS = read(ISfile)
        self.FS = read(FSfile)
        self.path = path
        self.bids = ids
    ...
    def run(self, calculator,fix_indices=list(range(32))):
        ...
        sigma_R1,diff_IS,diff_FS = directionality_sigma(RIS,RFS,R1Copt,R1,self.bids)
        ...
        if D_criteria(diff_IS,diff_FS) == True:
            ...
            sigma_R2,diff_IS_R2,diff_FS_R2 = directionality_sigma(R1Copt,Rref,R2Copt,R2,self.bids,outshow=['alpha','ref'])
            ...
            if sigma_R2 == 'ref' :
                ...
                while sigma_Ri == 'ref' or sigma_Ri == 'Nondirectional':
                    ...
                    sigma_Ri,diff_IS_Ri,diff_FS_Ri = directionality_sigma(R1Copt,Rref,RiCopt,Ri,self.bids,outshow=['alpha','ref'])
            ...
            while Euclidean_distance(Rgamma,Rref,self.bids) >= Euclidean_distance(Rdnc,Rref,self.bids):
                gamma = gamma + 0.1
...
for name in folderpath:
    ...
    if os.path.exists(f'{p0}/IntermediateProcess'): 
        if check_file_exists(f'{p0}/IntermediateProcess/results/','TS_RDA_S_R2Copt.xyz') or check_file_exists(f'{p0}/IntermediateProcess/results/','TS_RDA_S_RdncCopt.vasp') or check_file_exists(f'{p0}/IntermediateProcess/results/','TS_RDA_S_R1Copt.vasp'):
            ...
            SearchTS = RDA_S(ISfile=f'{path}/{name}/IntermediateProcess/optimized_IS_FS/IS_opt.vasp', FSfile=f'{path}/{name}/IntermediateProcess/optimized_IS_FS/FS_opt.vasp', path=p0, ids=answerlist[1])
            SearchTS.opt(calc)
            SearchTS.run(calc)
    else:
