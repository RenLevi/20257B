...
class RDA_S():
    ...
    def run_idpp(self, mlp_path,fix_indices=list(range(32))):
        ...
        if D_criteria(diff_IS,diff_FS) == True:
            ...
            self.control_rtrust_by_fmax(Sella_Search)
            ...
            if sigma_R2 == 'ref' :
                ...
                R2final =  [copy.deepcopy(BETAlist[-1]),'Rdc']
            elif sigma_R2 == 'alpha':
                sigma_Ri = copy.deepcopy(sigma_R2)
                BETAlist.append(beta)
                ...
                R2final = [copy.deepcopy(BETAlist[-1]),'Rdnc']
            else:
                BETAlist.append(beta)
                sigma_Ri = copy.deepcopy(sigma_R2)
                while sigma_Ri == 'Nondirectional':
                    beta = beta + 0.1
                    Ri,_ = interpolate_structure_from_idpp(initial_file=R1Copt,final_file=Rref,n_images=11,fraction=beta,output_file=f'{path_IP}/step2/R2N_{int(beta*100)}.vasp')
                    print(f"当前beta(N): {beta}")
                    RiCopt, _ = optimize_with_energy_criterion(Ri, calc, 
                                                                energy_threshold=0.05,
                                                                max_steps=200,
                                                                #trajectory_file= f'{path_IP}/step2/R2_{int(beta*100)}_Copt.traj',
                    ...
                    write(f'{path_IP}/step2/R2N_{int(beta*100)}_Copt.xyz', RiCopt)
                    sigma_Ri,diff_IS_Ri,diff_FS_Ri = directionality_sigma([R1Copt,Rref],[RiCopt,Ri],Determination=['alpha','ref','Nondirectional'])
                    print(f"RiCopt(N)的diff_IS: {diff_IS_Ri}; diff_FS: {diff_FS_Ri}, sigma_Ri: {sigma_Ri}")
                    BETAlist.append(beta)
                    assert beta >= 0 , "beta < 0 ,程序终止!"
                beta2 = beta
                if sigma_Ri == 'ref' :
                    BETAlist.append(beta2)
                    sigma_Rii = copy.deepcopy(sigma_R2)
                    while sigma_Rii == 'ref' or sigma_Rii == 'Nondirectional':
                        beta2 = beta2 - 0.1
                        Rii,_ = interpolate_structure_from_idpp(initial_file=R1Copt,final_file=Rref,n_images=11,fraction=beta2,output_file=f'{path_IP}/step2/R2_{int(beta2*100)}.vasp')
                        print(f"当前beta: {beta2}")
                        RiiCopt, _ = optimize_with_energy_criterion(Rii, calc, 
                                                                    energy_threshold=0.05,
                                                                    max_steps=200,
                                                                    #trajectory_file= f'{path_IP}/step2/R2_{int(beta*100)}_Copt.traj',
                                                                    #log_file=  f'{path_IP}/step2/R2_{int(beta*100)}_Copt.log'
                                                                    )
                        write(f'{path_IP}/step2/R2_{int(beta2*100)}_Copt.xyz', RiiCopt)
                        sigma_Rii,diff_IS_Rii,diff_FS_Rii = directionality_sigma([R1Copt,Rref],[RiiCopt,Rii],Determination=['alpha','ref','Nondirectional'])
                        print(f"RiiCopt的diff_IS: {diff_IS_Rii}; diff_FS: {diff_FS_Rii}, sigma_Ri: {sigma_Rii}")
                        BETAlist.append(beta2)
                        assert beta2 >= 0 , "beta < 0 ,程序终止!"
                    #Rdc_beta =  copy.deepcopy(BETAlist[-1])
                    R2final = [copy.deepcopy(BETAlist[-1]),'Rdc']
                elif sigma_R2 == 'alpha':
                    sigma_Rii = copy.deepcopy(sigma_R2)
                    BETAlist.append(beta2)
                    while sigma_Rii == 'alpha'or sigma_Rii == 'Nondirectional':
                        beta2 = beta2 + 0.1
                        Rii,_ = interpolate_structure_from_idpp(initial_file=R1Copt,final_file=Rref,n_images=11,fraction=beta2,output_file=f'{path_IP}/step2/R2_{int(beta2*100)}.vasp')
                        print(f"当前beta: {beta2}")
                        RiiCopt, _ = optimize_with_energy_criterion(Rii, calc, 
            ...
            print(f"Rdnc_beta: {R2final[0]},direction: {R2final[1]}")
            #Rdc = interpolate_structure(R1Copt,Rref,fraction=Rdc_beta, output_file=f'{path_IP}step2/Rdc.vasp')
            Rd,_ = interpolate_structure_from_idpp(initial_file=R1Copt,final_file=Rref,n_images=11,fraction=R2final[0], output_file=f'{path_IP}step3/Rd.vasp')
            RdCopt, _ = optimize_with_energy_criterion(Rd, calc, 
                                                            energy_threshold=0.05,
                                                            max_steps=100,
                                                            #trajectory_file= f'{path_IP}step3/RdncCopt.traj',
                                                            #log_file=  f'{path_IP}step3/RdncCopt.log'
                                                            )
            write(f'{path_IP}step3/RdncCopt.xyz',RdCopt)
            direction = R2final[1]
            if direction == 'Rdc':
                if sigma_R1 == 'IS':
                    print("将RFS作为Rref")
            ...
            gamma = 0.1
            R3,_ = interpolate_structure_from_idpp(initial_file=RdCopt,final_file=Rref,n_images=11,fraction=gamma,output_file=f'{path_IP}step3/R3_{int(gamma*100)}.vasp')
            Rgamma = copy.deepcopy(R3)
            while Euclidean_distance(Rgamma,Rref) >= Euclidean_distance(Rd,Rref):
                gamma = gamma + 0.1
                Rgamma,_ = interpolate_structure_from_idpp(initial_file=RdCopt,final_file=Rref,n_images=11,fraction=gamma,output_file=f'{path_IP}step3/R3_{int(gamma*100)}.vasp')
            ...
            print("过渡态搜索完成,结果保存在TS.xyz")
            return QTS
    def control_rtrust_by_fmax(self,opt,control_delta=[0.01,5e-3,5e-4],fmax_limit=[0.05,0.06,0.1]):
        for i, geom in enumerate(opt.irun(fmax=fmax_limit[0])):
                _, fmax, cmax = opt.pes.converged(opt.fmax)
                print(f'step {i} fmax : {fmax}')
                if fmax < fmax_limit[2] and opt.delta > control_delta[0]:
                    opt.delta = control_delta[0]
                if fmax < fmax_limit[1] and opt.delta > control_delta[1]:
                    opt.delta = control_delta[1]
        return opt
