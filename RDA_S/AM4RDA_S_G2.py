...
for batch in batch_process(fdl,11):
    ...
    with open(f'test/jobsub/{FLODER}/{i}/config.json','w') as j:
        json.dump(data,j)
    sp.copyFiles('RDA_S/RDA_S_G2.py',f'test/jobsub/{FLODER}/{i}')
    sp.copyFiles('RDA_S/SearchTS_G2.py',f'test/jobsub/{FLODER}/{i}')
    sp.copyFiles('RDA_S/jobRDA_S_G2.sh',f'test/jobsub/{FLODER}/{i}')
    sp.run_command_in_directory(directory=f'test/jobsub/{FLODER}/{i}',command='qsub jobRDA_S_G2.sh')
    i += 1