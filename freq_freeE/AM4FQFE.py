...
for batch in batch_process(fdl,11):
    ...
    with open(f'test/jobsub/{folder}/{i}/config.json','w') as j:
        json.dump(data,j)
    sp.copyFiles('freq_freeE/freq_freeE.py',f'test/jobsub/{folder}/{i}')
    sp.copyFiles('freq_freeE/jobfreq_freeE.sh',f'test/jobsub/{folder}/{i}')
    sp.run_command_in_directory(directory=f'test/jobsub/{folder}/{i}',command='qsub jobfreq_freeE.sh')
    i += 1