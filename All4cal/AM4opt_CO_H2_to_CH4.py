...
with open ('test/RN/mol_without_C.txt','w') as f:
    ...
    f.write('O\n')
    f.write('OO\n')
smiles2ase = s2m.smi2mol(input='test/RN/mol_without_C.txt',output='test/output/molecule')
smiles2ase.mollist2SMILES()
smiles2ase.creat_folder_and_file()
smiles2ase.build_model()
...
molecule2system = m2a.mol2ads(
    input='test/output/molecule/species_name.json',
    output='test/output/system',
    metal='test/output/slab/Ru_hcp0001.vasp')
molecule2system.creat_folder_and_file()
molecule2system.build_model(smi2molPATH='test/output/molecule/species',random_mol_num=1,size=(4,4,4))

#MLP优化结构#并列执行
OPT4ALLSYS = mlpopt.opt4ALLsystems_batch_by_batch(input='test/output/system',random_number=1)
OPT4ALLSYS.start_split(10)
foldersplitlist = OPT4ALLSYS.fsp
for i in range(len(foldersplitlist)):
    fl = foldersplitlist[i]
    data = {
        'path':'/work/home/ac877eihwp/renyq/20250828TT/test/output/system',
        'record':'/work/home/ac877eihwp/renyq/20250828TT/test/output/system/record_adscheck.json',
        'folderpath':fl,
        'random_number':1
        }
