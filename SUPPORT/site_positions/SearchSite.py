...
class SurfaceSiteFinder:
    ...
    def find_sites(self, contact_distance: float = 2, multi_site_threshold: float = 2):
        """
        识别表面位点
    ...
    def visualize(self, show_grid: bool = False, show_wrapped: bool = True):
        ...
        # 绘制原子
        indices_to_mol = [atom.index for atom in self.atoms if atom.index > 47]
        atoms = self.atoms[indices_to_mol]
...
if __name__ == "__main__":
    ...
    # 查找位点
    sites, positions, vectors = finder.find_sites(contact_distance=2.5)
