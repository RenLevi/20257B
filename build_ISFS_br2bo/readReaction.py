...
from build_ISFS_br2bo.CheckNN import *
        ...
        for i, j, limit_dist, mode in self.distance_constraints:
            ...
            if mode == 0:
                ...
                if distance <= limit_dist:
                    ...
                    #print(f"调整意外靠近原子对 ({i},{j}) 受力，距离: {distance:.4f} Å")
        return forces
...
class readreaction():
    ...
    def run_MDAO(self,path,MLPs_model_path):
                ...
                if i == j:pass
                    ...
                    if check_NON_metal_atoms(twogroups[i]) == False or check_NON_metal_atoms(twogroups[j])==False:pass
                        ...
                        if changebondatom in [(i,j),(j,i)]:
                            distance_constraints.append((i, j,3, 1))
                        else:
