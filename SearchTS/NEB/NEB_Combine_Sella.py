...
def run_neb(IS, FS, nImg, out_file, p,mlp_path = None):
    steps = 2000
    a = False
    while not a:
        a = cycle_neb(IS, FS, nImg, steps, p, mlp_path)
        nImg += 1
...
def cycle_neb(IS, FS, nImg, steps, p, mlp_path = None):
    a = [IS.copy() for i in range(nImg+1)] + [FS]
    for i in a: 
        i.calc = NequIPCalculator.from_deployed_model(mlp_path, device='cpu')
...
for name in dictrn:
    ...
    def CHECKISFS(dictrns,name):
        rnd = dictrns[name]
        keylist = list(rnd.keys())
        if "final ads" in keylist:
            ...
            return False
    def warp(IS,FS,p,mlp_path = None):    
        print(f'Start NEB')
        TS = run_neb(IS, FS, 8, 'TS', p, mlp_path)
        d = [IS, TS, FS]
        dn=['IS','TS','FS']
        print('-'*50)
        for i in d:
        ...
        initial_structure.set_calculator(NequIPCalculator.from_deployed_model(mlp_path, device='cpu'))
    ...
    if CHECKISFS(dictrn,name):
        IS = read(f'{path}/{name}/IS.vasp')
        FS = read(f'{path}/{name}/FS.vasp')
        warp(IS,FS,PathName,model_path)
        with open(f'{path}/{INAME}.json','r') as j:
            dictij = json.load(j)
